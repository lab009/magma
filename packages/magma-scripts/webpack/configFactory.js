...
export default function webpackConfigFactory(_options) {
  ...
  webpackConfig
    ...
    .entry('index')
      ...
      .when(isDevClient, use => use.add(`${require.resolve('webpack-hot-middleware/client')}?reload=true&path=http://${config('host')}:${config('clientDevServerPort')}/__webpack_hmr`))
    ...
    .output
      ...
      .when(isDev, use => use.publicPath(`http://${config('host')}:${config('clientDevServerPort')}${config('bundles.client.webPath')}`), use => use.publicPath(bundleConfig.webPath))
      .end()
    .when(isClient,
    ...
    .resolve
      ...
      .extensions
        .merge(config('bundleSrcTypes').map(ext => `.${ext}`))
        .end()
      .modules
        .add('local_modules')
        .add('node_modules')
  ...
  if (isDev) {
    ...
    webpackConfig.plugin('error')
      .use(webpack.NoEmitOnErrorsPlugin)
  }

  if (isDevClient) {
    // We need this plugin to enable hot reloading of our client.
    webpackConfig.plugin('hot')
  ...
  webpackConfig.plugin('happypack-javascript')
  .init((Plugin, babelConfig) => happyPackPlugin({
  ...
  .when(isDevClient, use => use.set('args', [
  ...
  .when(isProdClient, use => use.set('args', [
    require.resolve('@lab009/babel-preset-magma'),
    {
      runtime: true,
    },
  ...
  if (isDevClient) {
    ...
    webpackConfig.plugin('happypack-devclient-css')
  ...
  // JAVASCRIPT
  webpackConfig.module.rule('javascript')
    .test(/\.jsx?$/)
    .include
      .add(/local_modules/)
  ...
  // LOOKUP
  webpackConfig.module.rule('lookup')
    .test(/\.lookup$/)
    .use('glob')
  ...
  if (isClient || isServer) {
    // CSS
    webpackConfig.module.rule('style')
      .test(/\.css$/)
